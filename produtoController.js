...
exports.buscarProduto = (req, res) => {
    ...
    db.query('SELECT * FROM produto WHERE id = ?', id_produto, (err, result) => {
        if (err) {
            console.error('Erro ao buscar produtos:', err);
            res.status(500).json({ error: 'Erro interno no servidor' });
            return;
...
exports.adicionarProduto = (req, res) => { //token
    const { id_produto, nome_produto, descricao, valor, imagem,medida } = req.body;

    const { error } = produtoSchema.validate({ id_produto, nome_produto, descricao, valor, imagem,medida });
...
exports.atualizarProduto = (req, res) => { //token
    ...
    db.query('UPDATE produto SET ? WHERE id = ?', [produtoAtualizado, id_produto], (err, result) => {
        if (err) {
...
exports.deletarProduto = (req, res) => { //token
    ...
    db.query('DELETE FROM produto WHERE id = ?', id_produto, (err, result) => {
        if (err) {
